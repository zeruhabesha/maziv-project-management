...
const Projects: React.FC = () => {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-xl shadow-sm border p-4">
        <div className="flex flex-col sm:flex-row gap-4">
          <div className="flex-1 relative">
            ...
            <input type="text" placeholder="Search by client name..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="pl-10 p-2 w-full border rounded-lg bg-gray-50 focus:bg-white" />
          </div>
          <div className="sm:w-48 relative">
            <Filter className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-gray-400" />
            <select value={statusFilter} onChange={(e) => { setStatusFilter(e.target.value); setCurrentPage(1); }} className="pl-10 p-2 w-full border rounded-lg appearance-none bg-gray-50 focus:bg-white">
      ...
      {loading && projects.length === 0 && <p className="text-center p-4">Loading projects...</p>}
              ...
              <div className="p-6">
                <div className="flex items-start justify-between mb-4">
                  <div>
                    ...
                    <h3 className="text-lg font-semibold text-gray-900 mt-2 hover:text-blue-600"><Link to={`/projects/${project.id}`}>{project.name}</Link></h3>
                    <p className="text-sm text-gray-500">{project.client} (#{project.ref_no})</p>
                  </div>
                  <div className="flex items-center space-x-1">
                    {canCreateOrEdit && <button onClick={() => handleEditProject(project)} className="p-2 text-gray-400 hover:text-blue-600 rounded-lg" title="Edit"><Edit className="h-4 w-4" /></button>}
                    {isAdmin && <button onClick={() => handleDeleteProject(project.id, project.name)} className="p-2 text-gray-400 hover:text-red-600 rounded-lg" title="Delete"><Trash2 className="h-4 w-4" /></button>}
                    <button
                    ...
                    {project.file && (
                      <a
                        href={`/uploads/projects/${project.file}`}
                        target="_blank"
                ...
                <div className="mb-4">
                  <div className="flex items-center justify-between text-sm mb-1"><span className="font-medium text-gray-700">Progress</span><span className="font-semibold text-blue-600">{project.stats?.progress_percentage || 0}%</span></div>
                  <div className="w-full bg-gray-200 rounded-full h-2.5"><div className="bg-blue-600 h-2.5 rounded-full" style={{ width: `${project.stats?.progress_percentage || 0}%` }}></div></div>
                </div>
                <div className="flex items-center justify-between text-sm text-gray-500 border-t pt-4 mt-4">
                  <div className="flex items-center space-x-1.5"><Calendar className="h-4 w-4" /><span>Ends: {new Date(project.end_date).toLocaleDateString()}</span></div>
                  <div className="flex items-center space-x-1.5 font-medium text-gray-700"><DollarSign className="h-4 w-4 text-green-500" /><span>{formatCurrency(project.tender_value)}</span></div>
                </div>
              </div>
            </motion.div>
          ))}
      ...
      {/* TODO: Add Pagination controls here if pagination.pages > 1 */}

      <CreateProjectModal isOpen={showCreateModal} onClose={() => setShowCreateModal(false)} />
      {selectedProject && <EditProjectModal isOpen={showEditModal} onClose={() => setShowEditModal(false)} project={selectedProject} />}
      {assignProject && (
        <AssignProjectModal
